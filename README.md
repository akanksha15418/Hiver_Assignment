...
This project is a Java Spring Boot application that acts as an AI support agent for processing customer support tweets for AppleSupport. It is built using LangChain4j and the Gemini API, strictly adhering to Java constraints (no Python) and utilizing an in-memory vector store for RAG.
...
5. **Test the Pipeline:**
   Send a POST request to `/api/support/handle`:
   ```bash
   curl -X POST http://localhost:8080/api/support/handle \
   -H "Content-Type: application/json" \
   -d '{"message": "I was charged twice for my app store purchase!"}'
   ```
...
1. **Sarcasm Misinterpretation:**
   - *Example:* "Oh great, another update that bricks my phone. Thanks Apple!"
...
3. **Missing Context in RAG:**
   - *Example:* "Error code 4005 on iTunes."
...
4. **Vague Inquiries:**
   ...
   - *Hypothesis:* Lack of product entity recognition. The agent drafts a generic reply instead of asking clarifying questions about the device type.
5. **Hallucination on Links:**
   - *Example:* AI provides `apple.com/support/link-1234`.
...
4. **Gemini API:** Chosen for high quality and speed, especially `gemini-1.5-flash` which is highly cost-effective for RAG workflows.
